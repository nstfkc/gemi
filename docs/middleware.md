...
Entries are aliases (with `alias:param` arguments, as in a route's list) or middleware classes, and they run in order. They run once for every request the server receives, before anything else:
