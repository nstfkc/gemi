...
- **`gemi dev` and a shared table.** `gemi dev` runs `bun --hot`, which re-evaluates modules in the same process; a queue loop started before a save keeps running, because a live closure still holds it. With the memory driver each stale loop polls its own empty `Map` and is merely invisible. With `driver: "database"` every one of them polls the same `gemi_jobs` table, so after a few saves several generations of stale code are claiming and running real rows side by side. Restart `gemi dev` when you are working on jobs against a database driver, or point development at `"memory"`.
