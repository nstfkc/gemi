...
Upgrades an app from the 0.45 service-provider layout to the 0.50 config + container layout.
...
> This command only makes sense once, when moving from 0.45 to 0.50. It is a no-op on an app that has no `app/kernel/providers/` directory.
