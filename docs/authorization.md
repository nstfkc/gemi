...
>
> An app upgrading from 0.62, where it answered `401`, can keep that on API routes while its
> shipped clients catch up: `InsufficientPermissionsError.apiStatus = 401`, set once at module
> scope in `app/kernel/Kernel.ts`. See UPGRADE.md.
