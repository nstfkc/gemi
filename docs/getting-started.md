...
- **[`@vitejs/plugin-react`](https://github.com/vitejs/vite-plugin-react)** — required by the client/SSR build and registered in your app's `gemi.config.ts` (the scaffolded template includes it). The template turns on the React Compiler through it, which additionally needs `oxc-transform-react`; see [React Compiler](./configuration.md#react-compiler).
