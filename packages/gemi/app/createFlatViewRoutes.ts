    ...
    if ("run" in viewConfigOrViewRouter) {
      ...
      if ("children" in route) {
        ...
        for (const [path, { exec, middleware }] of Object.entries(result)) {
          ...
          // `routePath` is passed along — view/layout routes use it for the
          // breadcrumb href, file and redirect routes for error messages.
          const handler = (req: HttpRequest<any, any>) => route.run.call(route, req, routePath);
      ...
      for (const [path, { exec, middleware, viewPath }] of Object.entries(result)) {
        const key = routePath === "/" ? path : `${routePath}${path}`;
        const _key = path === "/" && routePath !== "/" ? routePath : key;
        flatRoutes[removeGroupPrefix(_key)] = {
          exec,
