...
The queue slice's `driver` accepts any `QueueDriver`, and one backed by storage
that outlives the process changes this. Check `app/config/queue.ts` before relying
on either answer; absent a `driver`, it is memory.
